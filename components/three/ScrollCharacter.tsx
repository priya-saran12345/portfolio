...
function SoldierModel() {
  ...
  useEffect(() => {
    gltf.scene.traverse((object) => {
      ...
      const monochromeMaterials = originalMaterials.map((material) => {
        ...
        if (
          ...
          /*
           * Convert every material to a neutral grayscale value.
           * We intentionally remove the color/albedo texture because the
           * Soldier GLB texture contains the original colorful uniform.
           */
          const sourceColor = cloned.color ?? new THREE.Color("#888888");
...
function Scene() {
  ...
  return (
    <>
      {/* Pure monochrome lighting */}
      <hemisphereLight
        args={["#ffffff", "#090909", 1.85]}
      />
